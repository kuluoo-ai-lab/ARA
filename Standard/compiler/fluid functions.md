...
├─ Fixed intent and purpose
├─ Preset conditions and rules
├─ Deterministic logic paths
├─ Immutable function signature
└─ Controlled execution environment

        ↓ UNIFIED THROUGH FLUID ARCHITECTURE ↓
...
<fluid_function>
    ...
    ║  purpose = Classify weather conditions    ║
    ║                                            ║
    ║  input = {                                 ║
    ...
    ║                                            ║
    ║  output = {                                ║
    ║      classification: string (static type) ║
    ║      confidence: float      (static type) ║
    ║  }                                         ║
    ║                                            ║
    ║  STATIC CHARACTERISTICS:                   ║
    ║  • Type signatures (frozen)                ║
    ║  • Function structure (fixed)              ║
    ║  • Flow pattern (6-step, immutable)        ║
    ║  • Error handling strategy (predetermined) ║
    ║  • Caching rules (static)                  ║
    ║                                            ║
    ╚════════════════════════════════════════════╝
```
...
- ✅ Input/output types
- ✅ Flow structure (STEP-1 through STEP-6)
...
<fluid_function>
    ...
    ║      model = "gpt-4"     (STATIC)          ║
    ║      temperature = 0.2   (STATIC)          ║
    ║  }                                         ║
    ║                                            ║
    ║  system_prompt = """                       ║
    ║      You are a weather expert.             ║
    ║      Analyze these metrics...              ║
    ║      Consider current weather patterns...  ║ (DYNAMIC)
    ║      Adapt to seasonal variations...       ║ (DYNAMIC)
    ║      Learn from historical data...         ║ (DYNAMIC)
    ║  """                                       ║
    ║                                            ║
    ║  DYNAMIC CHARACTERISTICS:                  ║
    ║  • AI model reasoning (adaptive)           ║
    ║  • Context-aware decisions (mutable)       ║
    ║  • Parameter interpretation (flexible)     ║
    ║  • Condition evaluation (ever-changing)    ║
    ║  • Response generation (intelligent)       ║
    ║  • Learning from patterns (evolving)       ║
    ║                                            ║
    ╚════════════════════════════════════════════╝
```
...
├─ Generate function skeleton (STATIC)
├─ Implement type system (STATIC)
├─ Generate flow steps (STATIC)
├─ Embed LLM integration (DYNAMIC)
├─ Inject system prompt (DYNAMIC)
└─ Add parameter handling (DYNAMIC)
        ↓
[Generate Code]
...
- humidity: float ✓ (frozen)
- wind_speed: float ✓ (frozen)

OUTPUT TYPES:
- classification: string ✓ (frozen)
- confidence: float ✓ (frozen)
...
✓ Invalid ranges → runtime validation error
✓ Missing fields → error
✓ Type integrity throughout execution
...
GUARANTEES:
✓ Will never exceed max_retries
✓ Will never exceed rate_limit
✓ Will never exceed cost_limit
✓ Will timeout after 10 seconds
✓ No more than 10 concurrent calls
```
...
- Current global weather patterns     ← CHANGES DAILY
- Seasonal variations                 ← CHANGES BY SEASON
- Historical trends                   ← EVOLVING DATA
- Real-time conditions                ← ALWAYS DIFFERENT

Your reasoning:
- May discover new patterns           ← LEARNING
- May adapt to edge cases             ← ADAPTIVE
- May combine multiple factors        ← INTELLIGENT
- May provide nuanced analysis        ← CONTEXTUAL
"""
...
Winter Context (Jan):
- "22.5°C is WARM for winter"
- "65% humidity is MODERATE"
- "15 km/h is LIGHT wind"
→ Interpretation: "Unusually warm day"

Summer Context (July):
- "22.5°C is COOL for summer"
- "65% humidity is COMFORTABLE"
- "15 km/h is GENTLE breeze"
→ Interpretation: "Pleasant mild day"
...
Call 1: input=(10, 40, 5)    → Output: "CLEAR SKY" (confidence: 0.98)
Call 2: input=(20, 60, 15)   → Output: "CLOUDY" (confidence: 0.85)
Call 3: input=(5, 90, 30)    → Output: "STORMY" (confidence: 0.92)
Call 4: input=(35, 30, 2)    → Output: "SUNNY" (confidence: 0.99)
...
STATIC COMPILE-TIME (Before Execution):
  ...
  7. Generate language-specific code (static)
     ...
     ├─ ClassifyWeather class (structure)
     ├─ __init__ method (initialization)
     ├─ execute method (flow)
     ├─ validate_input method (validation)
     ├─ prepare_llm_request method (preparation)
     └─ All other methods (static skeleton)


DYNAMIC RUNTIME (During Execution):
  1. User calls: classifier.execute(22.5, 65, 15)
  2. STATIC: Validate input types (float, float, float) ✓
  3. STATIC: Check input ranges (temperature in [-50, 50]) ✓
...
```
╔════════════════════════════════════════════════════════════════╗
║          FLUID FUNCTION CONVERTER (Agent Syntax Compiler)       ║
╚════════════════════════════════════════════════════════════════╝
...
✓ Static structure (type-safe, predictable)
✓ Dynamic intelligence (AI-powered, adaptive)
✓ Production readiness (error handling, monitoring)
✓ Full functionality (all 6 steps implemented)
```
...
2. VALIDATE_STATIC_COMPONENTS(parsed)
   ├─ Validate function_name is unique
   ├─ Validate input types are valid
   ├─ Validate output types are valid
   ├─ Validate flow has exactly 6 steps
   ├─ Validate error_handling strategy
...
3. VALIDATE_DYNAMIC_COMPONENTS(parsed)
   ├─ Validate llm_config.provider in [openai, anthropic, cohere, huggingface, local]
   ├─ Validate llm_config.model exists for provider
   ├─ Validate system_prompt is non-empty
   ├─ Validate temperature is 0.0-1.0
...
4. GENERATE_STATIC_SKELETON(parsed, targetLanguage)
   ...
   Create class/struct:
   ├─ Class name from function_name
   ├─ Constructor/init method
   ├─ Config class with static settings
   ├─ Metrics class for monitoring
   ├─ execute() method signature
   │  ├─ Parameters from input definition
   │  └─ Return type from output definition
   ├─ Validation methods
   │  ├─ validate_input() - checks types and ranges
   │  ├─ validate_output() - checks types and constraints
   │  └─ validate_cache_key() - validates cache
...
5. GENERATE_DYNAMIC_ENGINE(parsed, staticSkeleton)
   ...
   Enhance with:
   ├─ LLM client initialization
   │  └─ Choose correct SDK based on provider
   ├─ System prompt embedding
   │  └─ Inject system_prompt into model
   ├─ API call implementation
   │  ├─ Format request for provider
   │  ├─ Handle response format
   │  └─ Extract model output
   ├─ Parameter interpretation
   │  └─ Dynamic handling of input variations
   ├─ Adaptive response handling
   │  └─ Parse variable response formats
...
6. INTEGRATE_STATIC_DYNAMIC(staticSkeleton, dynamicEngine)
   ...
   Merge:
   ├─ Static structure provides framework
   ├─ Dynamic engine fills intelligence
   ├─ Static validation wraps dynamic calls
   ├─ Static error handling wraps dynamic errors
   ├─ Static caching stores dynamic results
   ├─ Static monitoring tracks dynamic metrics
   └─ Execute method orchestrates both
...
7. GENERATE_LANGUAGE_SPECIFIC_CODE(integratedCode, targetLanguage)
   ...
   ├─ Import required libraries (openai, anthropic, etc.)
   ├─ Define classes with proper Python syntax
   ├─ Implement type hints
   ├─ Add docstrings
   ├─ Implement error handling with try/except
   ├─ Create async methods where needed
   ├─ Implement logging
   └─ Generate usage examples
   
   For targetLanguage = "typescript":
   ├─ Define interfaces for types
   ├─ Create classes with TS syntax
   ...
   ├─ Implement methods with receivers
   ├─ Handle errors with error type
   ├─ Use goroutines if appropriate
...
10. WRITE_TO_FILE(finalCode, outputPath)
    ...
    ├─ Write config file (if separate)
    ├─ Write test file (if requested)
    ├─ Write README (if requested)
    ├─ Write requirements.txt/package.json (if needed)
    └─ Return file path
...
12. RETURN COMPILED_AGENT(outputPath)
    ...
    ├─ Agent is ready for import/use
    ├─ All static guarantees met
    ├─ All dynamic intelligence integrated
    ├─ Production ready
...
✓ Type definitions (frozen)
✓ Validation rules (immutable)
✓ Flow order (6 steps, unchanging)
✓ Error handling (predetermined)
✓ Configuration values (static constants)
✓ Execution guarantees (deterministic)

DYNAMIC ASPECTS (Runtime):
✓ Input values (always different)
✓ LLM responses (intelligent, adaptive)
✓ Decision logic (context-aware)
✓ Output values (unique each time)
✓ Reasoning process (AI-powered)
✓ Adaptation (responsive to conditions)
✓ Learning patterns (evolving)
✓ Real-time intelligence (always current)

UNIFIED BEHAVIOR:
✓ Static structure ensures reliability
✓ Dynamic intelligence ensures capability
✓ Together they create adaptive agent
✓ Predictable framework + intelligent execution
✓ Type-safe + AI-powered
"""
```
...
├─ Type-safe + AI-smart
├─ Maintainable + Powerful
├─ Debuggable + Capable
└─ Production-ready + Cutting-edge
```
...
**Fluid Functions: Where yesterday's reliability meets today's intelligence!** 🚀
